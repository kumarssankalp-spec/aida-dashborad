...
import { FiX, FiMail, FiDollarSign, FiPercent, FiUser, FiCheckCircle, FiSend, FiClock, FiCheck } from 'react-icons/fi';
          ...
          <motion.div initial={{ scale: 0.8, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.8, opacity: 0, y: 20 }} transition={{ type: 'spring', duration: 0.5 }}>
            <Card className="bg-white shadow-xl border border-gray-200 max-w-md w-full mx-4">
              <CardContent className="p-8 text-center">
                ...
                <h2 className="text-2xl font-bold gradient-text mb-4">Confirmation Sent!</h2>

                <p className="text-gray-600 mb-6 leading-relaxed">Thank you for confirming your project proposal. We've sent the details to your email and will contact you shortly to discuss the next steps.</p>

                <div className="p-4 rounded-lg bg-gradient-to-r from-[#c8e9f7]/20 to-[#8676e9]/10 border border-white/20 mb-6">
                  <div className="flex items-center justify-center space-x-2 text-[#8c52ff] mb-2">
                    <FiClock className="w-5 h-5" />
                    <span className="font-semibold">Next Steps</span>
                  </div>
                  <p className="text-sm text-gray-600">Our team will review your confirmation and reach out within 24 hours to schedule a project kickoff meeting.</p>
                </div>

                <Button onClick={onClose} className="w-full bg-gradient-to-r from-[#8c52ff] to-[#8676e9] hover:from-[#8676e9] hover:to-[#8c52ff] text-white font-semibold">Back to Dashboard</Button>
              </CardContent>
            </Card>
          </motion.div>
        </motion.div>
    ...
    <AnimatePresence>
      <motion.div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose}>
        <motion.div initial={{ scale: 0.8, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.8, opacity: 0, y: 20 }} transition={{ type: 'spring', duration: 0.5 }} onClick={(e) => e.stopPropagation()}>
          <Card className="bg-white shadow-xl border border-gray-200 max-w-lg w-full mx-4">
            <CardHeader className="pb-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h2 className="text-2xl font-bold gradient-text">Confirm Proposal</h2>
                    <p className="text-gray-600">Review and confirm project details</p>
                  </div>
                </div>
            ...
            <CardContent>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                  <label htmlFor="name" className="block text-base font-medium text-gray-700 mb-2">Client Name</label>
                  <div className="relative">
                    <FiUser className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-5 h-5" />
                    <Input id="name" type="text" value={formData.name} onChange={(e) => handleInputChange('name', e.target.value)} className="pl-10 glassmorphism border-2 border-white/30 focus:border-[#8c52ff]" required />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <Input id="email" type="email" value={formData.email} onChange={(e) => handleInputChange('email', e.target.value)} className="pl-10 glassmorphism border-2 border-white/30 focus:border-[#8c52ff]" required />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="relative p-3 pl-10 bg-gray-50 border border-gray-200 rounded-md">
                    <span className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400"><RiDiscountPercentFill/></span>
                ...
                <div className="flex space-x-4">
                  <Button type="button" onClick={onClose} variant="outline" className="flex-1 border-gray-900 text-gray-700 hover:bg-black" disabled={isSubmitting}>Cancel</Button>
