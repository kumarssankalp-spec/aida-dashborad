...
const Dashboard: React.FC<DashboardProps> = ({ onLogout }) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#c8e9f7] via-white to-[#8676e9]/20 relative overflow-x-hidden">
      ...
      {showConfirmation && (
        <ConfirmationModal onClose={() => setShowConfirmation(false)} clientData={{ name: currentClient.name, email: currentClient.email, price: projectData.proposalDetails.budget, discount: projectData.proposalDetails.discount }} />
      )}
    </div>
  );
};
